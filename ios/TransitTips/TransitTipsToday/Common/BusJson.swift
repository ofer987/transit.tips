...
public class BusJson {
    ...
    public static func toModel(_ json: Welcome) -> MySchedule {
        return convertToSchedule(json)
    }
    
    public static func convertToSchedule(_ source: Welcome) -> MySchedule {
        
        let location = convertToLocation(source.address, latitude: source.latitude, longitude: source.longitude)
        let result = MySchedule(location)
        
        var allStations = [String:MyStation]()
        ...
        for route in source.routes {
            if let station = allStations[route.stop.id] {
                ...
                let address = MyLocation(latitude: 0.0, longitude: 0.0)
                let station = MyStation(id, name, address)
    ...
    static func convertToLine(_ route: RouteElement) -> MyLine {
        var id = 0
        if let unwrappedId = route.route.id {
            id = Int(unwrappedId) ?? 0
        }
        let result = MyLine(id, route.route.title)
        
        for value in route.values {
            let direction = MyDirection(value.direction.title)
            direction.arrivals.append(MyArrival(value.minutes, value.seconds))
    ...
    static func convertToLocation(_ address: String, latitude: Double, longitude: Double) -> MyLocation {
