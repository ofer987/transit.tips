...
public class TrainJson {
    ...
    public static func toModel(_ json: Schedule) -> MySchedule {
    ...
    public struct Event: Codable {
        ...
        public func toModel() throws -> MyArrival {
            ...
            return MyArrival(minutes!, 0)
        }
        
        public func toOptionalModel() -> MyArrival? {
            ...
            return MyArrival(Int(minutes), Int(minutes.truncatingRemainder(dividingBy: 1) * 60))
        }
        
        public func toPreciseModel() -> MyArrival {
            let minutes = Int(self.preciselyIn)
            let seconds = Int(self.preciselyIn.truncatingRemainder(dividingBy: 1) * 60)
            
            return MyArrival(minutes, seconds)
        }
    }
    
    public static func convertToSchedule(_ source: Schedule) -> MySchedule {
        let result = MySchedule(MyLocation(latitude: source.latitude, longitude: source.longitude))
    ...
    static func convertToStation(_ source: Station) -> MyStation {
        let location = convertToLocation(
            latitude: source.latitude,
            longitude: source.longitude
        )
        
        return MyStation(0, source.name, location)
    }
    
    static func convertToLine(_ source: Line) -> MyLine {
        return MyLine(Int(source.id), source.name)
    }
    
    static func convertToDirection(_ source: Direction) -> MyDirection {
        return MyDirection(source.destinationStation)
    }
    
    static func convertToArrival(_ source: Event) -> MyArrival? {
        return try source.toOptionalModel()
    }
    
    static func convertToLocation(latitude: Double, longitude: Double) -> MyLocation {
